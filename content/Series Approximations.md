#Maths/Calculus, #Maths/Numerical-Methods   
# Maclaurin [[Series]]
...
The Maclaurin series for a function $f(x)$ that is infinitely [[Differentiation#Differentiability|Differentiable]] at $x = 0$ is given by:
...
This expansion only if $f^{(n)}(a)$ exists $\forall n \in \mathbb{N}$ and for values of $x$ for which the series [[Series#Convergence|Converges]].
