...
The more rigorous definition is that a set of vectors $\mathbf{v}_{k}$ is ***linearly dependant*** if there exists solution to the equation such that $a_{1},a_{2},\dots ,a_{k}$ are not all zero.

$$
a_{1}\mathbf{v}_{1}+a_{2}\mathbf{v}_{2}+\dots+a_{k}\mathbf{v}_{k}=0
$$
...
For equal vectors it makes sense to just choose their length squared so we get the property that $\langle \mathbf{v},\mathbf{v} \rangle=\lVert v \rVert^{2}$ and there is only one such inner product that satisfies this. For a vector space over $\mathbb{R}$ it is 

$$
\langle \mathbf{v},\mathbf{w} \rangle =\frac{1}{4}((\lVert \mathbf{v}+\mathbf{u} \rVert)^{2}-(\lVert \mathbf{v}-\mathbf{u} \rVert)^{2} )
$$
...
$$
\mathbf{p}\text{ s.t } \mathbf{p}\cdot \begin{pmatrix}x \\ y \\ z\end{pmatrix}=f\begin{pmatrix}x \\  y\\ z\end{pmatrix}
$$
