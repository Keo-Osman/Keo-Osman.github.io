...
Let $x_i$ be any point in $[x_{i-1}, x_i]$. Then $\int_a^b f(x)dx = \lim_{n \to \infty} \sum_{i=1}^n f(x_i) \Delta x$. *Provided the limit exists any choice of* $x_i^*$ *will yield the same result.*
...
---
# Properties
If $f$ is continuous on $[a,b]$ or $f$ has a finite number of jump discontinuities then $f$ is integrable on $[a,b]$

---
# Symmetry
Suppose $f$ is continuous on $[-a,a]$ then:
1. $f$ is even $\Rightarrow \int_{-a}^a f(x)dx = 2\int_0^a f(x)dx$
2. $f$ is odd $\Rightarrow \int_{-a}^a f(x)dx = 0$

---
# Comparisons
1. $f(x) > 0$ $\forall x \in [a,b] \Rightarrow \int_a^b f(x)dx > 0$
2. $f(x) > g(x)$ $\forall x \in [a,b] \Rightarrow \int_a^b f(x)dx > \int_a^b g(x)dx$
3. $m \leq f(x) \leq M$ $\forall x \in [a,b] \Rightarrow m(b-a) \leq \int_a^b f(x)dx \leq M(b-a)$ (integral is in between rectangles bounded by maximum and minimum)
...
$\frac{d}{dx} [F(g(x))] = F'(g(x)) \cdot g'(x)$ using $v = g(x)$
$\int f(g(x))g'(x) = F(g(x)) + C - F(v) + C = \int f(v)dv$
