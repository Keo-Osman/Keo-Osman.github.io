...
## Common Examples
For $r>0 \in \mathbb{Q} \Rightarrow \lim_{x\to\infty}\frac{1}{x^r}=0$

$r>0 \in \mathbb{Q}$ s.t. $x^r$ is defined $\forall x \Rightarrow \lim_{x\to\infty}\frac{1}{x^r}=0$
## Asymptotes
The line $y=L$ is a horizontal asymptote of $y=f(x)$ if either $\lim_{x\to\infty}f(x)=L$ or $\lim_{x\to-\infty}f(x)=L$
### Slant Asymptotes
